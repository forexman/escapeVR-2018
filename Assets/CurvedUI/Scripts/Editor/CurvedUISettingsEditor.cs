            ...
            if (Application.isPlaying)
            {
                ...
                foreach(var key in ControlMethodDefineDict.Keys)
                {
                    if(myTarget.ControlMethod == key && !define.Contains(ControlMethodDefineDict[key]))
                        Debug.LogWarning("CURVEDUI: Selected control method (" + key.ToString() + ") is not enabled. Enable it on CurvedUISettings component", myTarget.gameObject);
                }
            }
...
#if CURVEDUI_STEAMVR_LEGAC1Y || CURVEDUI_STEAMVR_LEGACY || CURVEDUI_GOOGLEVR || CURVEDUI_OCULUSVR
                //controller override
                GUILayout.Space(20);
        ...
        Dictionary<CurvedUIInputModule.CUIControlMethod, string> ControlMethodDefineDict {
            get
            {
                if (m_controlMethodDefineDict == null)
                {
                    ...
                    m_controlMethodDefineDict.Add(CurvedUIInputModule.CUIControlMethod.STEAMVR_LEGACY, "CURVEDUI_STEAMVR_LEGAC1Y");
                    m_controlMethodDefineDict.Add(CurvedUIInputModule.CUIControlMethod.STEAMVR_2, "CURVEDUI_STEAMVR_LEGACY");
